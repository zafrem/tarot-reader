...
- `src/data/deck.json` — the canonical, portable card dataset (78 entries: `name`, `arcana`, `number`/`suit`, `upright`, `reversed`, `images`), with no Python dependency. `images` is a dict keyed by theme (`{"default": "images/default/the_fool.jpg", "abstraction": "images/abstraction/the_fool.svg"}`), not a flat string — each theme gets its own subfolder under `src/data/images/`, add a sibling folder + matching key per card to add another. Two themes exist: `default/` (photographic public-domain Rider-Waite-Smith scans, `.jpg`, see `src/data/CREDITS.md`) and `abstraction/` (original SVG design, `.svg` — a small symbolic sigil near the top, drawn from real alchemical/astrological correspondences per suit/Major Arcana card, with the card name as the dominant large typographic element rather than an illustration; deliberately not AI-generic — no near-black+single-accent palette, no all-caps tracked labels, no middle-dot metadata strings). Both ship inside the built package (`pyproject.toml` `[tool.setuptools.package-data]` covers `data/images/**/*.jpg` and `**/*.svg`, recursive globs, specifically so per-theme subfolders are picked up automatically). A future native client (mobile, Obsidian plugin, etc.) can read this JSON and its images directly without any Python involvement — `tarot-web` already does exactly this via a git submodule.
